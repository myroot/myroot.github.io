...
comments: true
---
.net framework에서는 비동기 작업을 위해 여러가지 built-in된 방법을 제공하고 있으며, 특히 Task, await 그리고 async 등은 높은 수준의 개념으로  비동기 작업을 다룰수 있는 방법을 제공합니다. 이를 적극적으로 활용하면 비동기 작업을 위해 callback 지옥에 빠지는 등의 문제를 겪지 않고도 효율적인 비동기 코드를 작성할 수 있습니다.
...
await는 Task의 완료를 대기할 때, 어떤 Thread도 점유하지 않습니다. 그리고 Task가 완료된 이후에는 다시 돌아갈 Thread를 선택하고 해당 Thread에서 await 이후의 코드를 수행합니다.
 ...
 만약 SynchronizationContext.Current가 없는 상태의 Thread였다면 await 이후 코드는 새로운 Thread(worker thread)를 통해 수행이 됩니다. 그리고 Task를 완료하는 Thread가 Task를 완료한 후 돌아갈 Thread와 동일한 경우, 완료 후 수행해야할 작업이 event 큐에 Post하지 않고 Task를 완료하는 과정에 이어서 바로 (callstack이 쌓인체로) 수행 됩니다.

 예들 들면, TaskCompletionSource를 사용하여 작업 완료를 처리하려고 한 경우, 만약 await이후 돌아가려는 thread와 TasckCOmpletionSource에 SetResult를 통해 작업을 완료하려는 thread가 동일하다면, SetResult의 호출이 리턴되지 않은 상태에서 await 이후 코드가 수행되고 끝난 이후에야 SetResult의 호출이 리턴이 됩니다.

 그래서 만약 아래와 같은 코드가 있다면,
 ```c#
 TaskCompletionSource<bool> tcs = new TaskCompletionSource<bool>();
 SomeAsyncCallback(() => {
    Console.WriteLine("Before SetResult");
    tcs.SetResult();
    Console.WriteLine("After SetResult");
 });

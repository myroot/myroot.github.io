...
하지만 C#에서는 struct는 값 타입으로 동작하고 class는 참조 타입으로 동작하고 있고 이 둘간의 동작에 있어서 차이가 많이 있기 때문에 이 차이점을 잘 알고 표현하려는 객체에 적합한 타입을 선택해야 합니다.
...
이런 방식의 차이는 메소드의 인자로 인스턴스가 전달될 때도 동일한 방식으로 적용됩니다. 즉 struct타입의 인스턴스를 전달할때는 값이 복사되어 전달되며, class타입의 인스턴스를 전달할때는 참조만 전달이 됩니다. 따라서 메소드 안에서 전달된 인자에 대한 업데이트가 발생한 경우 strcut는 호출 시 사용된 인스턴스에는 영향을 미치지 않지만, class타입의 경우 호출 시 사용된 인스턴스에 영향을 미치게 됩니다.
